	...
	for (const hex of activeHexes) {
		switch (hex.state.type) {
			...
			case "university":
				...
				break;
			case "barracks":
				gameState.militaryBuildUp = Math.min(
					50,
					gameState.militaryBuildUp + 5
				);
				break;
			case "simpleHousing":
...
function moveWorkers(amt) {
	...
	if(curHex.workers + amt >= 0 && gameState.employed + amt <= gameState.population) {
		switch (curHex.type) {
			case "farm":
			case "factory":
			case "mine":
...
function buildButton(building) {
	if(gameState.actionPoints > 0 && g.Hexes[gameState.selectedHex].state.type == "none") {
		var isGucci = false;
		switch(building) {
			case "farm":
				...
				break;
			case "factory":
				if(gameState.stone >= 40 && gameState.iq >= 15 && gameState.population >= 50) {
					isGucci = true;
					gameState.stone -= 40;
					gameState.iq -= 15;
				}
			...
			case "mill":
				...
				break;
			case "school":
				if(gameState.stone >= 20 && gameState.wood >= 10 && gameState.population >= 30) {
					isGucci = true;
					gameState.stone -= 20;
					gameState.wood -= 10;
				}
				break;
			case "university":
				if(gameState.stone >= 30 && gameState.metal >= 30 && gameState.iq >= 15) {
					isGucci = true;
					gameState.stone -= 20;
					gameState.wood -= 10;
					gameState.iq -= 10;
				...
				break;
			case "barracks":
				if(gameState.stone >= 30 && gameState.iq >= 15 && gameState.metal >= 20 && gameState.population >= 30) {
					isGucci = true;
					gameState.stone -= 30;
					gameState.iq -= 15;
					gameState.metal -= 20;
			...
			case "simpleHousing":
				...
				break;
			case "superHousing":
				if(gameState.stone >= 40 && gameState.iq >= 15 && gameState.metal >= 30) {
					isGucci = true;
					gameState.stone -= 40;
					gameState.iq -= 15;
					gameState.metal -= 30;
				...
				break;
			case "cityHall":
				if(gameState.stone >= 15 && gameState.wood >= 15 && gameState.population >= 20) {
					isGucci = true;
					gameState.stone -= 15;
					gameState.wood -= 15;
				}
				break;
			case "nuclearFacilities":
				if(gameState.stone >= 60 && gameState.iq >= 70 && gameState.metal >= 90 && gameState.wood >= 20 && gameState.population >= 60) {
					isGucci = true;
					gameState.stone -= 30;
					gameState.iq -= 15;
					gameState.metal -= 20;
